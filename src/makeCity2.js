...
const makeCity2 = ({ scene, steps }) => {
  ...
  var roadMaterial = new THREE.MeshPhongMaterial({
    // create a new material from an asphalt texture
  ...
  var buildingBaseGeometry = new THREE.PlaneGeometry(
    buildingBaseSize,
    buildingBaseSize
  ); // create a square PlaneGeometry to use as a base under each building
  var buildingBaseMaterial = new THREE.MeshPhongMaterial({
    // create a new material from a base texture
  ...
  function AddBuildingBase(x, y, z) {
    var buildingBaseMesh = new THREE.Mesh(
      buildingBaseGeometry,
      buildingBaseMaterial
    ); // create a new mesh with the pre-setup geometry and material
  ...
  function AddRoads(x, y, z, width) {
    ...
    for (var b = 0; b < roadLocations.length; b++) {
      // loop through all of the road locations
      if (roadLocations[b].x == x && roadLocations[b].z == z) {
        // check if the current x,z coordinate is in the roadLocations array
        freeSpace = false; // the space as not free
      }
    }

    if (freeSpace) {
      // if this is a free space
      roadLocations.push({
  ...
  function AddLightPole(x, y, z) {
    ...
    loader.load("./assets/lightpole/light-pole.json", function (obj) {
      ...
      if (Math.round(Math.random()) == 0) {
        // randomly rotate the pole 90 degrees to create some variation
  ...
  function AddBuilding(startingX, startingZ, randomX, randomZ, stepsLeft) {
    ...
    for (var b = 0; b < buildingLocations.length; b++) {
      if (
        buildingLocations[b].buildingX == buildingX &&
        buildingLocations[b].buildingZ == buildingZ
      ) {
        freeSpace = false;
      }
    }
    ...
    if (freeSpace) {
      ...
      if (randBuilding < 75) {
        // 75% of the time a tall building will be chosen
        ...
        if (isWideRand < 5) {
          ...
          for (var b = 0; b < buildingLocations.length; b++) {
            if (
              buildingLocations[b].buildingX == buildingX + 100 &&
              buildingLocations[b].buildingZ == buildingZ
            ) {
              freeSpaceAdj = false;
            }
          ...
          for (var b = 0; b < buildingLocations.length; b++) {
            if (
              buildingLocations[b].buildingZ == buildingZ + 100 &&
              buildingLocations[b].buildingX == buildingX
            ) {
              freeSpaceAdj = false;
            }
        ...
        // don't make a smaller building double width
        AddBuildingBase(buildingX, 1.0084, buildingZ);
        AddBuild(
  ...
  function AddBuild(model, r, g, b, width, height, depth, xTra, yTra, zTra) {
    ...
    var mesh = null;
    loader.load("assets/building_models/b" + model + ".ply", function (
      geometry
    ) {
  ...
  function RandomWalk(startingX, startingZ, stepsLeft) {
    var randomRoadCoords = [
      // the 4 potential coordinates for a road to be placed
      [0, 1],
      [0, -1],
      [-1, 0],
      [1, 0],
    ...
    var randomBuildingCoords = [
      // the 4 potential coordinates for a building to be placed
      [0.5, 0.5],
      [0.5, -0.5],
      [-0.5, 0.5],
      [-0.5, -0.5],
    ...
    AddBuilding(
      startingX,
      startingZ,
      randomBuildingCoords[0],
      randomBuildingCoords[1],
      stepsLeft
    ); // Add a building at the randomly chosen offset

    var newStepsLeft = stepsLeft - 1; // decrement the amount of times left to recursively call
    if (newStepsLeft > 0) {
      // if there are still steps left to go
  ...
  function GenerateCity(steps) {
    for (var x = 0; x < steps; x++) {
      // call RandomWalk with the current number of steps
      RandomWalk(0, 0, x);
    }
  }
